  ...
  pandocBlocks.forEach(function (block) {
    ...
    switch (type) {
      ...
      case 'BlockQuote':
        ...
        blocks = blocks.concat(convertBlocks(value, level, context));
        break;
...
var convertTable = function(headers, rows, level, context) {
  var result = convertList(headers, 'bulleted', level, context);
  rows.forEach(function(row) {
    result = result.concat(convertList(row, 'bulleted', level, context));
  });
  return result;
};
