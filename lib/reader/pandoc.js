...
var convertInlines = function (pandocInlines, level, context) {
  var inlines = flattenInlines(pandocInlines, level, context, []);
...
var flattenInlines = function(pandocInlines, level, context, stylesStack) {
  var result = [];
  pandocInlines.forEach(function(inline) {
    ...
    switch (type) {
      ...
      case 'Emph':
        ...
        var inlines = flattenInlines(value, level, context, stylesStack.concat(type));
      ...
      case 'Strikeout':
        ...
        var inlines = flattenInlines(value, level, context, stylesStack);
        result = result.concat(inlines);
        break;
      ...
      case 'Link':
        ...
        var inlines = flattenInlines(value[0], level, context, ['Link']);
        result = result.concat(inlines);
        result.push(['linkref', linkRef]);
        var target = value[1];
        result.push(['link', { 'level': level }, ['linkref', linkRef], ['url', target[0]] ]);
        break;
