  ...
  pandocBlocks.forEach(function (block) {
    ...
    switch (type) {
      ...
      case 'Para':
        ...
        var textBlocks = convertInlines(value, level);
      ...
      case 'Header':
        ...
        var textBlocks = convertInlines(value[1], 0);
...
var convertInlines = function (pandocInlines, level) {
  var inlines = flattenInlines(pandocInlines, level, []);
  inlines = normalizeInlines(inlines);
  // TODO: extract notes and links (notes should be extracted recursively)
  // TODO: extract image and formula blocks
  return [['para', { 'level': level }].concat(inlines)];
};

var flattenInlines = function(pandocInlines, level, stylesStack) {
  var result = [];
  pandocInlines.forEach(function(inline) {
    ...
    switch (type) {
      ...
      case 'Emph':
        ...
        var inlines = flattenInlines(value, level, stylesStack.concat(type));
      ...
      case 'Strikeout':
        ...
        var inlines = flattenInlines(value, level, stylesStack);
        result = result.concat(inlines);
        break;
      ...
      case 'Link':
        ...
        // TODO: reserve and insert linkref
        // TODO: ensure that only one ‘linktext’ is created
        // TODO: handle case of literal url
        var inlines = flattenInlines(value[0], level, ['Link']);
        result = result.concat(inlines);
        var target = value[1];
        result.push(['link', { 'level': level }, ['url', target[0]] ]);
        break;
      ...
      case 'Note':
        ...
        // TODO: reserve and insert noteref
        var noteBlocks = convertBlocks(value, level);
        noteBlocks[0][0] = 'note';
        result.push(['note', noteBlocks ]);
        break;
